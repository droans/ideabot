...
class IdeabotAPI:
    ...
    def setup_routers(self) -> None:
        ...
        self.ideas_router = IdeasRouter(
            app=self._app, db=self._db, auth_fn=self.get_user
        )
        self._app.include_router(self.ideas_router.router)
        self.admin_router = AdminRouter(
            app=self._app, db=self._db, auth_fn=self.get_admin
        )
    ...
    def get_admin(self, api_key_header: str = Security(api_key_header)) -> str:
        ...
        if not admin:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Requires admin."
            )
        return user
