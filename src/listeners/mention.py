...
class MentionsListener:
    ...
    async def on_message(self, event: MessageCreate) -> None:
        ...
        logger.debug(
            f"Bot user {bot_user} {'' if bot_user in users else 'not'} in users {users}"
        )
        ...
        if not message.content.strip().startswith(bot_mention_prefix):
            logger.debug(
                "Bot may have been mentioned but message wasn't for bot, ignoring."
            )
            return

        original_message = await message.fetch_referenced_message()
        if original_message:
            _save_replied_message(
                self._bot,
                original_message=original_message,
                reply=message,
                bot_user_id=int(bot_user),
...
def parse_category_and_idea_name_from_reply(
    reply_content: str, bot_user_id: int
